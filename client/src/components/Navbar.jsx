...
const BookIcon = () =>(
    ...
    <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 19v4a1 2 0 0 1 2 2v16a2 2 0 0 1-2 2H4Z"/>
    </svg>
 )
const Navbar = () => {
    const navLinks = [
        { name: 'Home', path: '/' },
        { name: 'Tours', path: '/datve' },
    ...
    return (
            <nav className={`fixed top-0 left-0 bg-amber-400 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <div className="hidden md:flex items-center gap-4">
                    ...
                    "text-white bg-black" : "border-3 bg-transparent text-white"}`}>
                        Login
                    </button>)}
                ...
                {/* Mobile Menu Button */}
                <div className="flex items-center gap-3 md:hidden" >{user && <UserButton>
                        <UserButton.MenuItems>
                            <UserButton.Action label="My Tours" labelIcon={<BookIcon />} onClick={() => navigate('/my-tours')}/>
                        </UserButton.MenuItems>
                    </UserButton>}
