...
const AllTours = () => {
  ...
  return (
    ...
    justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px24 xl:px-32'>
        <div>
            <div className='flex flex-col items-start text-left'>
                <h1 className='font-playfair text-4xl md:text-[40px]'>Tours du lịch</h1>
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Hãy tìm kiếm những tour du lịch mà bạn mong muốn.</p>
            </div>

            {roomsDummyData.map((room)=>(
                <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6
                border-b border-b-gray-300 last:pb-30 last:border-0'>
                    <img onClick={() => {navigate(`/tours/${room._id}`),scrollTo(0,0)}}
                    ...
                    <p className='text-xl font-medium text-gray-700'>{room.pricePerNight} vnđ/người</p>
                  </div>
                </div>
            ))}
        ...
        <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
            <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-b-gray-300
                ${openFilters && "border-b"}`}>
                <p className='text-base font-medium text-gray-800'>Bộ Lọc</p>
                <div className='text-xs cursor-pointer'>
                    <span onClick={() => setOpenFilters(!openFilters)} className='lg:hidden'>{openFilters ? 'Ẩn' : 'Hiện'}</span>
